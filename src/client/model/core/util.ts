...
export const isThenble = (input: any): boolean =>
...
export const forcePlainDataCheck = (input: any, path = ''): void => {
  ...
  if (type === 'function') {
    throw new Error(
      `Expected plain data, but found a function in ${path}: ${input}`
    )
  }
  ...
  if (type === 'object') {
    if (!isPlainObject(input)) {
      throw new Error(
        `Expected plain object, but found an instance of ${input.constructor} in path ${path}: ${input}`
      )
    }
